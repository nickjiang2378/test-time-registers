...
def activate_on_registers(module, input, output, num_registers, neuron_indices, scale = 1.0, normal_values = "zero", bottom_frac = 0.75):
  ...
  if isinstance(scale, list):
    ...
    for i in range(num_registers):
      output[0, -num_registers + i, neuron_indices] = scale[i] * torch.max(output[0, :, neuron_indices], dim = 0).values
  else:
  ...
  if normal_values == "zero":
    ...
    # Set all image patch activations to the mean of the bottom_frac of activations
